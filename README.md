...
| Safe admin assistant | `/adminassist` proposes a public/private text channel, a no-permission role, a single-room jail-role plan, or an @everyone visibility edit. The initiating manager must confirm within five minutes; deletion, Administrator, webhooks, integrations, and edits to existing member or role permissions are refused. A jail-role plan creates a new empty role, limits that new role to one named text room, then stores it as the jail configuration. |
